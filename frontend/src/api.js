...
export const SampleApi = {
    getTaxiNum: (taxiId) => api.get("http://localhost:5000/api/taxi/" + taxiId),
    getList: () => api.get("http://localhost:5000/api/sample"),//샘플 리스트 가져오기 page2
    getInfo: (id) => api.get("http://localhost:5000/api/sample/" + id),//선택된 샘플 정보 가져오기
    getInfoImage: (id) => api.get("http://localhost:5000/api/sample/" + id + "/image"),
    postInfo: (body) => api.post("http://localhost:5000/api/review/", body)//택시번호, 샘플, 유저정보 디비에 저장
}
